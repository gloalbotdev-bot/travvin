...
export default function OwnerPanel() {
  ...
  return (
    <div
      className={`flex flex-col ${tab === 'home' || viewingZimmer ? 'h-dvh overflow-hidden' : 'min-h-screen'}`}
      dir="rtl"
    ...
    >
      ...
      <OwnerTopBar
        tab={viewingZimmer ? 'zimmers' : tab}
        primaryNav={primaryNav}
        onNav={(id) => { setViewingZimmer(null); handleNav(id); }}
        onAddBooking={() => setShowBookingCreator(true)}
        notifCount={notifCount}
        onBell={() => { setViewingZimmer(null); setTab('updates'); }}
        onSpark={() => { setViewingZimmer(null); setTab('assistant'); }}
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 lg:mb-8">
              <div className="text-right">
                <h1 className="font-simpler" style={{ color: '#0B3838', fontSize: 28, fontWeight: 600 }}>הצימרים שלי</h1>
                <p className="font-simona mt-1" style={{ color: '#717171', fontSize: 15 }}>{zimmers.length} נכסים</p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <button onClick={() => setShowAssistant(true)} className="font-simona flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-xl text-sm transition-all hover:opacity-90" style={{ background: 'rgba(11,56,56,0.08)', color: '#0B3838', border: '1.5px solid rgba(11,56,56,0.12)' }}>
                  <MessageSquare size={15} /><span className="hidden sm:inline">עוזר ניהול AI</span><span className="sm:hidden">עוזר AI</span>
                </button>
                <button onClick={() => setUsingCreatorChat(true)} className="font-simpler flex items-center gap-2 text-white px-3 sm:px-4 py-2.5 rounded-xl text-sm transition-all hover:opacity-90" style={{ background: '#0B3838', fontWeight: 600 }}>
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20">
                <div className="w-7 h-7 border-2 border-[#E4E7E5] border-t-[#0B3838] rounded-full animate-spin"></div>
              </div>
            ) : zimmers.length === 0 ? (
              <div className="text-center py-24 rounded-[23px] bg-white">
                <h3 className="font-simpler text-lg mb-2" style={{ color: '#0B3838', fontWeight: 600 }}>אין צימרים עדיין</h3>
                <p className="font-simona mb-6 text-sm" style={{ color: '#717171' }}>התחל על ידי הוספת הצימר הראשון שלך</p>
                <button onClick={() => setUsingCreatorChat(true)} className="font-simpler text-white px-5 py-2.5 rounded-full text-sm hover:opacity-90" style={{ background: '#0B3838', fontWeight: 600 }}>
                  הוסף צימר ראשון
                </button>
              </div>
            ) : (
...
function ZimmerListCard({ zimmer, onView, onEdit, onDelete }) {
  ...
  return (
    <div className="rounded-[23px] overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-lg cursor-pointer group bg-white"
      onClick={onView}>
      <div className="h-52 relative overflow-hidden" style={{ background: '#F8F7F4' }}>
        {img
          ...
          : <div className="w-full h-full flex items-center justify-center" style={{ background: '#EFEFEF' }} />
        }
        ...
        {zimmer.price_per_night && (
          <div className="font-simpler absolute bottom-3 right-3 text-white text-xs px-2.5 py-1 rounded-full" style={{ background: '#0B3838', fontWeight: 600 }}>
            ₪{zimmer.price_per_night}/לילה
          </div>
        )}
        {zimmer.approval_status && (
          <div className="absolute top-3 left-3">
            <span className="font-simona text-xs px-2 py-0.5 rounded-full"
              style={zimmer.approval_status === 'אושר'
                ? { background: '#EFEFEF', color: '#0B3838' }
                : { background: '#FFFF00', color: '#0B3838' }}>
              {zimmer.approval_status}
            </span>
          </div>
        )}
      ...
      <div className="p-4">
        <h3 className="font-simpler text-base mb-1 truncate" style={{ color: '#0B3838', fontWeight: 600 }}>{zimmer.name}</h3>
        <p className="font-simona text-sm mb-3 truncate" style={{ color: '#717171' }}>{zimmer.location || 'מיקום לא צוין'}</p>
        <div className="flex gap-3 font-simona text-xs mb-4" style={{ color: '#717171' }}>
        ...
        <div className="flex gap-2" onClick={e => e.stopPropagation()}>
          <button onClick={onEdit}
            className="font-simona flex-1 py-2 rounded-xl text-xs transition-all"
            style={{ background: 'rgba(11,56,56,0.08)', color: '#0B3838', fontWeight: 500 }}>
            עריכה
          </button>
          <button onClick={onDelete}
            className="font-simona px-3 py-2 rounded-xl text-xs transition-all"
            style={{ background: 'rgba(239,68,68,0.06)', color: '#EF4444', fontWeight: 500 }}>
            מחיקה
          </button>
        </div>
      </div>
